...
async def check_session_privacy(session_file, json_file, api_id, api_hash, privacy_settings_data, tdata_dir=None):
    start_time = time.time()
    log_time(f"开始隐私配置 session: {os.path.basename(session_file)}")
    
    temp_dir = tempfile.mkdtemp(prefix="yinsi_temp_")
    ...
    try:
        ...
        log_time(f"账号 {os.path.basename(session_file)} 隐私配置完成，状态={result['status']}，总耗时={total_time:.2f}秒")
        ...
        async def on_progress(t):
            ...
            for r in t.done:
                if r.get("category") in c:
                    c[r["category"]] += 1
                t._progress_text = f"""<tg-emoji emoji-id="5839200986022812209">🔄</tg-emoji> <b>{tr('privacy.in_progress', lang)}</b>
