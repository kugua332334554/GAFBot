...
async def check_session_kick(session_file, json_file, api_id, api_hash, tdata_dir=None):
    start_time = time.time()
    log_time(f"开始踢设备 session: {os.path.basename(session_file)}")
    
    temp_dir = tempfile.mkdtemp(prefix="tishebei_temp_")
    ...
    try:
        ...
        log_time(f"账号 {os.path.basename(session_file)} 踢设备完成，状态={result['status']}，总耗时={total_time:.2f}秒")
        ...
        async def on_progress(t):
            ...
            for r in t.done:
                if r.get("category") in c:
                    c[r["category"]] += 1
                t._progress_text = f"""<tg-emoji emoji-id="5839200986022812209">🔄</tg-emoji> <b>{tr('kick.in_progress', lang)}</b>
...
<tg-emoji emoji-id="5920052658743283381">✅</tg-emoji>{tr('shaihuo.success', lang)}: {c['success']} | <tg-emoji emoji-id="5922712343011135025">❌</tg-emoji>{tr('2fa.failed', lang)}: {c['fail']}"""
        ...
        def result_text_fn(cats, pending):
            return f"""<tg-emoji emoji-id="5909201569898827582">✅</tg-emoji> <b>{tr('kick.done', lang)}</b>
